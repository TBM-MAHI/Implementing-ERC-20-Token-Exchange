...
async function main() {
  ...
  /*   
    ...
    */
    ...
    //sender/deployer transfers 10,000 to receivers account
    amount = 10000;
    transaction = await MAHI.connect(deployerAccount).transfer(user1.address,convertToWei(amount));
    result = await transaction.wait();
    console.log(`Transferred ${amount} MAHI tokens from deployer's: ${deployerAccount.address} to user1: ${user1.address}\n`);
    
    transaction = await mETH.connect(deployerAccount).transfer(user2.address,convertToWei(amount));
    result = await transaction.wait();
    console.log(
      `Transferred ${amount} mETH tokens from deployer's: ${deployerAccount.address} to user 2: ${user2.address}\n`
    );
    ...
    */
    ...
    result = await transaction.wait();
    console.log(`Approved ${amount} tokens from user1: ${user1.address}`)
    //user1 deposits approved tokens
    transaction = await exchange.connect(user1).depositTokens(MAHI.address, convertToWei(amount));
    result = await transaction.wait();
    console.log(`Deposited ${amount} MAHI tokens from user 1 :${user1.address}\n`)
    ...
    result = await transaction.wait()
    console.log(`Order made from User 1:${user1.address}`);
   ...
   // console.log(result.events[0])
    ...
    result = await transaction.wait();
    console.log(`Cancelled order created from User 1:${user1.address}\n`);
    //WAIT
    await wait(2);
    ...
    result = await transaction.wait();
    console.log(`User 2 : Filled 1st order created from  User 1:${user1.address}\n`);
  ...
  // User 1 makes another order
    ...
    result = await transaction.wait()
    console.log(
      `User 2 : Filled 2nd order created from  User 1:${user1.address}\n`
    );
    ...
    result = await transaction.wait()
    console.log(
      `User 2 Filled 3rd order created from  User 1:${user1.address}\n`
    );
    ...
    // User 1 makes 10 orders --yet to be filled
    let tokenCount = 5;
    for (let index = 1; index < 10; index++) {
        ...
        orderID = result.events[0].args.id; 
        console.log(
          `\t Made order from  User 1: ${user1.address}  order No -${orderID}`
        );
        await wait(1);
    }
    // User 2 makes 10 orders-- yet to be filled
    for (let index = 1; index < 10; index++) {
        ...
        orderID = result.events[0].args.id; 
        console.log(
          `\t Made order from  User 2: ${user2.address}  order No -${orderID}`
        );
        await wait(1);
    }
}
